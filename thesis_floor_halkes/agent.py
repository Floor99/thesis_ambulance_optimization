...
class AmbulanceAgent:
    def __init__(self, policy: PolicyNetworkGATDynamicAttention, baseline=None, lr: float = 1e-3, gamma: float = 0.99):
        ...
        self.log_probs = []
        self.rewards   = []
        self.collected_states = []        # for critic baseline, store states to compute baselines
    ...
    def select_action(self, obs):
        ...
        # Store input state for baseline (if needed)
        self.collected_states.append(data)  # Pass full graph if critic expects it
    ...
    def finish_episode(self):
        ...
        # Normalize returns before critic loss
