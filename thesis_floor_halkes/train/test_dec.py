...
class GATModelEncoderStatic(nn.Module):
    def __init__(self, in_channels, hidden_size, out_size=None, num_layers=2, heads=4, dropout=0.2, edge_attr_dim=None):
        super().__init__()
        out_size = out_size or hidden_size
        self.conv1 = GATConv(in_channels, hidden_size, heads=heads, dropout=dropout, edge_dim=edge_attr_dim)
        self.conv2 = GATConv(hidden_size * heads, out_size, heads=1, dropout=dropout, edge_dim=edge_attr_dim)
...
class AttentionDecoderChat(nn.Module):
    ...
    def forward_step(self, node_embeddings, current_node_idx, invalid_action_mask=None):
        query = self.query_proj(node_embeddings[current_node_idx]).unsqueeze(0).unsqueeze(0)
...
def select_action(state, static_encoder, dynamic_encoder, decoder):
    static_emb = embed_graph((state.static_data.x_static, state.static_data.x_dynamic, state.static_data.edge_index, state.static_data.edge_attr), static_encoder, "static")
    dynamic_emb = embed_graph((state.dynamic_data.x_static, state.dynamic_data.x_dynamic, state.dynamic_data.edge_index, state.dynamic_data.edge_attr), dynamic_encoder, "dynamic")
    final_emb = torch.cat((static_emb, dynamic_emb), dim=1)

    action, log_prob, logits = decoder.forward_step(final_emb, state.current_node, state.visited)
    return action, log_prob

# ----- Build Sample Graph -----
def create_test_graph(num_nodes=6):
    edge_index = torch.tensor([[0, 1, 2, 3, 4, 0, 2, 3],
                               [1, 2, 3, 4, 5, 2, 4, 5]], dtype=torch.long)
