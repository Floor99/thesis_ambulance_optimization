...
class AmbulanceAgent:
    def __init__(
        ...
        self.optimizer = torch.optim.Adam(
            list(policy.parameters()) + self.baseline.get_learnable_parameters(),
            lr=lr
        )
        self.gamma     = gamma
    ...
    def finish_episode(self):
        ...
        discounts = self.gamma ** torch.arange(rewards.size(0), device=rewards.device, dtype=rewards.dtype)
        returns = (rewards * discounts[:,None]).flip(0).cumsum(0).flip(0) / discounts[:,None]
        # Now returns is [T, B]
